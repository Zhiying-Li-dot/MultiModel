...
| 方案 | 描述 | 优点 | 缺点 |
|---|---|---|---|
| A. 目标首帧重复 | `Zt_tar` 全部用目标首帧 | 内容一致 | 丢失运动信息 |
| B. 运动偏移迁移 | 源视频运动 + 目标首帧 | 保留运动 | latent 运动偏移无语义 |
| C. 两阶段方法 | Flux.2 + 纯 TI2V | 简单有效 | 不保留源视频结构 |
| D. FlowAlign | 3-branch，不依赖图像条件 | 保留结构 | 无法使用目标首帧 |

**结论**：TI2V + FlowEdit（有图像条件）方案存在理论矛盾，当前最佳方案仍是 **Flux.2 + TI2V 两阶段组合**。
